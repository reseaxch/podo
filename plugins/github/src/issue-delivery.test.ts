...
describe("GitHubIssueDeliveryAdapter", () => {
  ...
  test("continues to page two when a full provider page contains pull requests", async () => {
    const delivery = request()
    const marker = `<!-- podo-issue idempotency-key="${delivery.artifact.idempotencyKey}" draft-id="${delivery.artifact.id}" content-sha256="${delivery.artifact.contentSha256}" -->`
    ...
    const adapter = new GitHubIssueDeliveryAdapter({
      ...
      fetch: async (input, init) => {
        if ((init?.method ?? "GET") === "POST") {
          posts += 1
          return Response.json(issue(99, String(init?.body)), { status: 201 })
        }
        const url = new URL(String(input))
        requestedPages.push(url.searchParams.get("page") ?? "missing")
        if (url.searchParams.get("page") === "1") {
          return Response.json([
            ...Array.from({ length: 99 }, () => ({ pull_request: {} })),
            issue(8, "unrelated issue body"),
          ])
        }
        return Response.json([issue(12, `${content.body}\n\n${marker}`)])
      },
    })

    await expect(adapter.deliver(delivery)).resolves.toMatchObject({
      status: "existing",
      issue: { number: 12, url: "https://github.com/reseaxch/podo/issues/12" },
    })
    expect(requestedPages).toEqual(["1", "2"])
    expect(posts).toBe(0)
  })

  test("fails closed without creating when the bounded search cannot prove absence", async () => {
    const requestedPages: string[] = []
    let posts = 0
    const adapter = new GitHubIssueDeliveryAdapter({
      token,
      ...
      fetch: async (input, init) => {
        if ((init?.method ?? "GET") === "POST") {
          posts += 1
          return Response.json(issue(100, "must not create"), { status: 201 })
        }
        const url = new URL(String(input))
        requestedPages.push(url.searchParams.get("page") ?? "missing")
        return Response.json(Array.from({ length: 100 }, () => ({ pull_request: {} })))
      },
    })

    await expect(adapter.deliver(request())).rejects.toMatchObject({ code: "github_read_failed" })
